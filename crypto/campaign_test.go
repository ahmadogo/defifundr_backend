...
func createCampaign(t *testing.T) (*bind.TransactOpts, string, error) {
	...
	auth, campaign, err, c := CreateCampaign(title, campaignType, description, goal, deadline, image, private, "0e0c554d2b37105838b45e8b5a49d0edc9b00a8f")
	if err != nil {
		return nil, "", err
	}
	t.Log(c)
...
func TestCreateCampaignUnderPriced(t *testing.T) {
	configs, err := utils.LoadConfig("./../")
	require.NoError(t, err)
	require.NotEmpty(t, configs)
	...
	_, _, err, _ = CreateCampaign(title, campaignType, description, goal, deadline, image, private, address)
	require.Error(t, err)
}
