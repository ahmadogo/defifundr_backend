...
contract CrowdFunding {
    ...
    function createCampaign(
        ...
        require(bytes(_campaignType).length > 0, "Campaign type is required");
        require(bytes(_title).length > 0, "Campaign title is required");
        require(
            bytes(_description).length > 0,
            "Campaign description is required"
        );
        require(_goal > 0, "Campaign goal is required");
        require(_deadline > 0, "Campaign deadline is required");
        require(bytes(_image).length > 0, "Campaign image is required");
